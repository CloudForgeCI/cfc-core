...
InteractiveDeployer (the same path a user takes), then checks the stack status, the resources created, and
...
Environment: LOCALSTACK_AUTH_TOKEN must be set. CFC_CLASSPATH overrides the Java classpath (default
target/classes:target/dependency/*). Run from cfc-testing.
"""
import argparse
import csv
import datetime
...
def load_platform_module():
    spec = importlib.util.spec_from_file_location("localstack_platform", ROOT / "scripts" / "localstack-platform.py")
...
def deploy_row(row, base, platform):
    ...
    if not platform.restart_clean():
        return {"deploy": "FAILED: LocalStack did not become healthy", "checks": "", "detail": "", "summary": None}

    started = time.time()
    try:
        with open(log_path, "w") as log:
            subprocess.run(["java", "-cp", CLASSPATH, "com.cloudforgeci.samples.app.InteractiveDeployer",
                            "--context", str(context_path), "8"],
...
def main():
    ...
    for index, row in enumerate(todo, 1):
        ...
        outcome = deploy_row(row, base, platform)
