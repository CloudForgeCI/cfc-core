...
public final class FargateRuntimeConfiguration implements RuntimeConfiguration {
  ...
  public void wire(SystemContext c) {
    ...
    // 2c) Service behind HTTPS - wait for all components to be ready
    ...
    // 2d) HTTP listener already has correct default action (redirect for SSL, fixed response for no SSL)
    // No need to modify the default action here since AlbFactory creates it correctly
