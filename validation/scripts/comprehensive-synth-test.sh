...
# BASE_DIR should be validation/ (parent of scripts/)
...
# Function to parse cdk-nag CSV reports for a given rule level ("Warning" or "Error")'s
# Non-Compliant findings. cdk-nag writes these report files to $CDK_OUT_DIR regardless of how
# synthesis was invoked (in-process via cloudforge-cli, or the cdk CLI), so this is the reliable
# source for compliance findings -- unlike grepping console output for cdk-nag's "[Warning at
# ...]"/"[Error at ...]" annotation text, which only the cdk CLI binary itself ever prints (an
# in-process app.synth() call never renders it). Returns count found and prints them.
parse_cdknag_csv_findings() {
    local stack_name="$1"
    local rule_level="$2"
    local icon="$3"
    ...
    for csv_file in "$CDK_OUT_DIR"/*-"${stack_name}"-NagReport.csv "$CDK_OUT_DIR"/*NagReport.csv; do
        if [ -f "$csv_file" ]; then
            ...
            # Parse CSV for the requested rule level + Non-Compliant entries
            # CSV format: Rule ID,Resource ID,Compliance,Exception Reason,Rule Level,Rule Info
            while IFS=',' read -r rule_id resource_id compliance exception row_level rule_info; do
                ...
                if [[ "$row_level" == "$rule_level" && "$compliance" == "Non-Compliant" ]]; then
                    ((finding_count++))
                    # Print first 10 findings
                    if [ $finding_count -le 10 ]; then
                        echo "      $icon [$pack_name] $rule_id: $rule_info"
                    fi
                fi
    ...
    if [ $finding_count -gt 10 ]; then
        echo "      ... and $((finding_count - 10)) more"
    fi
...
run_synthesis() {
    ...
    # Use cloudforge-cli with deployment-context.json (written by create_deployment_context)
    ...
    if [ -n "$timeout_cmd" ]; then
        $timeout_cmd 300 cloudforge-cli deploy --context "$BASE_DIR/deployment-context.json" \
            --synth-only --outdir "$CDK_OUT_DIR" > "$synth_output" 2> "$synth_error"
    else
        cloudforge-cli deploy --context "$BASE_DIR/deployment-context.json" \
            --synth-only --outdir "$CDK_OUT_DIR" > "$synth_output" 2> "$synth_error"
    fi
    ...
    if [ -f "$CDK_OUT_DIR/$stack_name.template.json" ]; then
        ...
        # Parse cdk-nag's own *-NagReport.csv files (written to $CDK_OUT_DIR regardless of how
        # synthesis was invoked) for L1 errors and warnings, instead of grepping console output
        # for cdk-nag's "[Error at ...]"/"[Warning at ...]" annotation text -- that format is
        # rendered by the cdk CLI binary itself reading the assembly manifest, which never runs
        # here (cloudforge-cli calls CloudForgeSynthesizer.synthesize() in-process).
        local has_errors=false
        local has_advisories=false
        local advisory_layers=""

        local csv_errors=""
        csv_errors=$(parse_cdknag_csv_findings "$stack_name" "Error" "❌" 2>&1)
        local csv_error_count=$?
        if [ $csv_error_count -gt 0 ]; then
            has_errors=true
        fi

        local csv_warnings=""
        csv_warnings=$(parse_cdknag_csv_findings "$stack_name" "Warning" "⚠" 2>&1)
