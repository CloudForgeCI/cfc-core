...
# Synthesizes templates through cloudforge-cli and tracks synthesis timing metrics. No changeset
# is created (that requires AWS credentials and an existing stack); template analysis stands in
# as the deployment-readiness proxy. Suitable for cron scheduling for continuous validation.
...
run_dry_run_deployment() {
    ...
    if ! cloudforge-cli deploy --context "$BASE_DIR/deployment-context.json" --synth-only \
            --outdir "$CDK_OUT_DIR" > "$synth_log" 2> "$synth_error"; then
