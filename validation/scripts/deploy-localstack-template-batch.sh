...
# Requires cloudforge-core and cloudforge-api already built (mvn install) -- cloudforge-localstack
# itself is resolved as a regular published dependency (see cfc-core/pom.xml), not built from a
# sibling checkout, so its classpath comes from cloudforge-api's own resolved dependencies below.
#
...
# cloudforge-api's own resolved dependency classpath already includes cloudforge-localstack (a
# compile-scope dependency of cloudforge-api) and everything it needs at runtime -- reusing it
# here avoids hardcoding jar/version paths or assuming a sibling cloudforge-localstack checkout.
DEPS_CP_FILE="$(mktemp)"
STAGING_DIR="$(mktemp -d)"
trap 'rm -f "$DEPS_CP_FILE"; rm -rf "$STAGING_DIR"' EXIT
mvn -q -o -f ../cloudforge-api/pom.xml dependency:build-classpath "-Dmdep.outputFile=$DEPS_CP_FILE"
CP="../cloudforge-core/target/classes:../cloudforge-api/target/classes:$(cat "$DEPS_CP_FILE")"
